...
RidgePlot <- function(city_abbrev,              # Melb or Syd
  ...
  if(plot_only == FALSE){ 
    ...
    all_obs <- all_obs %>% 
      mutate(rel.weight.disagg = frac/max(frac)) %>% # this helps us distinguish large groups for setting alpha (fade)
      mutate(rel.weight.agg = frac.agg/max(frac.agg)) # this is for the aggregated groups
  ...
  ggplot(all_obs, aes(x = dist, y = inc_agg, alpha = rel.weight.agg)) +
    
    stat_density_ridges(quantile_lines = TRUE, # this makes ridge plots
                        quantiles = c(0.5, 0.8), # pick percentiles of interest
                        fill = gpal(1),
                        scale = overlap, # make this larger for overlapping ridges, e.g. 1.5
                        rel_min_height = 0.01) + #kills the density when it's too low, like a japanese painting
    
    theme_grattan() +
    
    theme(axis.text.y = element_text(vjust = 0), # aligns category labels with y gridlines
          legend.position = "right", # adds a legend in the same spot, same size each time
